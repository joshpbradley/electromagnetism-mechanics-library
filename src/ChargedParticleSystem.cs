...
namespace ElectromagnetismMechanicsLibrary
{
    ...
    public class ChargedParticleSystem : MonoBehaviour
    {
        ...
        /// </summary>
        [System.Serializable]
        internal sealed class ChargeBoundPair : System.IEquatable<ChargeBoundPair>
        {
            /// <summary>
            ...
            public override int GetHashCode()
            {
                ...
                hash = hash * 23 + _boundOne.GetHashCode();
                hash = hash + 23 + _boundTwo.GetHashCode();
            ...
            {
                return Equals(obj as ChargeBoundPair);
            }
        ...
        /// </summary>
        List<Vector4> _chargeAndBData = new();

        /// <summary>
        /// Used to denote that a neutral charge for a particle has been assigned a charge value. This is arbitrary but must be a non-integer value.
        /// </summary>
        ...
        void Awake()
        {
            ...
            _emittedParticles = new ParticleSystem.Particle[_attachedParticleSystem.main.maxParticles];
            ...
            foreach (var cb in _chargeBounds)
            {
                cb.BoundSwap();
            }

            _chargeBounds = _chargeBounds.Distinct().ToArray();
        }
        ...
        void FixedUpdate()
        {
            ...
            int numParticlesAlive = AttachedParticleSystem.GetParticles(_emittedParticles);

            AttachedParticleSystem.GetCustomParticleData(_chargeAndBData, ParticleSystemCustomData.Custom1);

            UpdateChargeData();
            UpdateBData();

            AttachedParticleSystem.SetCustomParticleData(_chargeAndBData, ParticleSystemCustomData.Custom1);
			 ...
			 */
            for (int i = 0; i < AttachedParticleSystem.particleCount; i++)
            {
                if (_chargeAndBData[i].w == NEUTRAL_FLAG || _emittedParticles[i].totalVelocity.magnitude == 0) continue;

                // The external forces module multiplier scales the effect's strength.
                /*
				 ...
				 */
                _emittedParticles[i].velocity += AttachedParticleSystem.externalForces.multiplier * _chargeAndBData[i].w * Vector3.Cross(_chargeAndBData[i], _emittedParticles[i].totalVelocity);
            }

            // Sets the new velocities calculated for the _attachedParticleSystem member.
            AttachedParticleSystem.SetParticles(_emittedParticles, numParticlesAlive);
        }
        ...
        void UpdateChargeData()
        {
            ...
            for (int i = 0; i < _chargeAndBData.Count; i++)
            {
				 ...
				 */
                _chargeAndBData[i] = new(cabd.x, cabd.y, cabd.z, charge == 0 ? NEUTRAL_FLAG : charge);
            }
        }
        ...
        void UpdateBData()
        {
            ...
            for (int i = 0; i < _chargeAndBData.Count; i++)
            {
                ...
                foreach (var magneticDipole in SubscribedMagneticDipoles)
                {
                    B += magneticDipole.CalculateBVectorAtPosition(_emittedParticles[i].position);
                }
        ...
        public bool SimulationCheck()
        {
            ...
            if (!AttachedParticleSystem.externalForces.enabled)
            {
            ...
            {
                Debug.LogWarning("A magnetic dipole hasn't been subscribed to a charged particle system instance.");
                canSimulate = false;
            }

            if (_chargeBounds.Length == 0)
            {
                Debug.LogError("At least one pair of charge bounds must be provided in the inspector.");
                canSimulate = false;
            }

            if (AttachedParticleSystem.main.simulationSpace != psss)
            {
                Debug.LogError("The Particle System component's Simulation Space must be set to World space.");
                canSimulate = false;
            }
        ...
        public bool SetChargeBounds(List<(int, int)> replacementBounds)
        {
            ...
            _chargeBounds = chargeBoundsTemp.Distinct().ToArray();
            ...
            {
                var cabd = _chargeAndBData[i];
                _chargeAndBData[i] = new(cabd.x, cabd.y, cabd.z, 0);
            }
